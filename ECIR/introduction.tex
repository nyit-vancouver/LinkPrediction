...
In this work we study the problem of predicting relationships in a DHIN, which can be formulated as: \textit{Given a DHIN graph $G$ at $t$ consecutive time intervals, we predict the existence of a particular relationship/path between two given nodes at time t + 1}. To this end, the main contributions of our work include:

\amin{Since it is a "problem" I think it's better to present it as a question. "....Given a DHIN graph $G$ at $t$ consecutive time intervals, how can we predict the existence of a particular relationship/path between two given nodes at time $t+1$?"} %The main contributions of our work include:
...
\item We propose the problem of relationship prediction in a DHIN, and draw a contrast between between this problem and existing link prediction techniques that have been proposed for dynamic and heterogeneous networks;
