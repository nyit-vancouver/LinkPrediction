...
\ebrahim{If you are raising this point, then you have to also say why pp+ does not work well on movie dataset?
Also, I could not see how the justification to combine features was reached. You may want to discuss more.}
...
\caption{The ROC curves for different methods and datasets.\ebrahim{please change the style of the lines so that it would be visible even if printed in BW.}} \label{fig:auc}
\end{figure}
...
\textit{The Effect of Dimensionality}. For this experiment we only compare prediction performance of MetaDynaMix with $t$=7 on the Publications dataset with respect to different values of $k$. Our results show that for $k$=5, 10, and 20, we get AUCROC values %0.84, 0.86, and 0.87, and ACC values 0.8, 0.82, and 0.82, respectively.
 A, B, and 0.87, and ACC values C, D, and 0.82, respectively.\amin{todo: update A,B,C, and D.}
 
 \ebrahim{two things: 1. The values are missing. 2. You need to write a bit of explanation about the finding when you have the actual numbers.}
