...
Sun et al. \cite{sun2011ASONAM} showed that their proposed \textit{PathPredict} approach is superior to traditional link prediction techniques such as common neighbors, Jaccard?s coefficient, and Katz. Such methods are homogeneous and non-temporal. The number of common neighbors \cite{newman2001clustering}, preferential attachment \cite{newman2001clustering}, Jaccard's coefficient \cite{liben2007link}, and Katz$\beta$ \cite{katz1953new}, are amongst frequently used topological features defined in homogeneous networks.

Since the path count results are not considerably different than PathSim, we consider comparing with path count due to efficiency in meta path-based feature calculation.
...
Mc Nemar's test [12, 13] is a variant of c2 test and is a non-parametric test used to analyse matched pairs of data. According to Mc Nemar?s test, two algorithms can have 4 possible outcomes arranged in a 2×2 contingency table [14] as shown in Table 3.

Using this criterion, the z scores are calculated usingMc Nemar?s test for the five classification algorithms. All the algorithms were used with their default parameters as parameter tuning may favor one algorithm to produce better results. The null hypothesis (H0) for this experimental design suggests that different classifiers perform similarly whereas the alternative hypothesis (H1) claims otherwise suggesting that at least one of the classifiers performs differently.
