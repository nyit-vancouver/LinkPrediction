...
To asses link prediction performance, we use Area Under Curves (AUC) for Receiver Operating Characteristic (ROC) and Precision-Recall (PR) \cite{davis2006relationship}, and accuracy (ACC). %We also present the prediction accuracy for 5-fold cross validation. 
...
\textit{Link Prediction Accuracy}. In this part we evaluate prediction capabilities of different methods. Results shown in Figure \ref{fig:auc} is based on setting the number of time intervals $t$ to 7 for dynamic methods and 3 intervals for PathPredict. Table \ref{tbl:auc} shows more details considering different intervals. These results indicated the superiority of MetaDynaMix prediction method compared to the rest. We also observe that PathPredict2 is better compared to LRHLP in predicting links for the publications network but not for the movies network. This to some extends indicate that the latent features for the movies network are more predictive compared to meta path-based ones.
...
%\subfigure[Publications Network]{
%\includegraphics[trim = 0mm 5mm -10mm 0mm,width=0.4\hsize]{figs/ROC.pdf}
%\includegraphics[trim = -10mm 5mm 0mm 0mm,width=0.4\hsize]{figs/ROC.pdf}
% \label{Fig:DBLP2}
%}
%\subfigure[Movies Network]{
%\includegraphics[trim = 0mm 5mm -10mm 10mm,width=0.4\hsize]{figs/ROC.pdf}
%\includegraphics[trim = -10mm 5mm 0mm 10mm,width=0.4\hsize]{figs/ROC.pdf}
% \label{Fig:IMDB}
%}
...
\multicolumn{1}{|l|}{\multirow{2}{*}{PathPredict2}}  & ROC  &  0.78  &   0.80    &     0.83    &   0.56   &    0.57     &    0.57     \\ \cline{2-8}
...
\textit{Significance of Performance Improvement}.


McNemar's Test to compare the predictive accuracy of two models


The null hypothesis indicates that none of the two models performs better than the other. Thus, the alternative hypothesis is that the performances of the two models are not equal.

 The null hypothesis of the McNemar's states that the same proportion of links will be correctly classified by the two methods. However the test result gives a $p$-value $<$ 0.0001 and hence we reject the null hypothesis of equal classifier performance.
...
\textit{The Effect of Time Intervals}. We set the number of time interval to $t$ to 3, 5, and 7 and assess its impact on prediction performance. As presented in Table \ref{tbl:auc} accuracy increases with the number of snapshots. Similarly authors in \cite{Zhu2016} showed that shorter time interval results in less changes in graphs and thus more reliable prediction.
...
\textit{The Effect of Dimensionality}.\amin{TBA}
