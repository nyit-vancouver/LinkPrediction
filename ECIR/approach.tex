...
Recent research in link prediction have focused on network latent space inference  \cite{Zhu2016,ye2013predicting,qi2013link,dunlavy2011temporal,menon2011link} with the assumption that the probability of a link between two nodes depends on their positions in their latent space. Each dimension of the latent space characterizes an attribute, and the more two nodes share such attributes, the more likely they are to connect (also known as homophily). Amongst such graph embedding methods, a few  \cite{dunlavy2011temporal,Zhu2016} considered dynamic networks. Inspired by %the matrix factorization approach presented by 
...
As an alternative solution, we build a predictive model that considers a linear combination of topological and latent features interpolation. %These features, however, can be interpolated in different ways that is beyond the scope of this work. 
