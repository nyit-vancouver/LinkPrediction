...
public static void main(String[] args)
{
	...
	for (int i = 0; i < 1; i++)
		try
	{
			FileInputStream in = new FileInputStream("paper.txt");
			...
			if (Character.isLetter((char) ch))
			{
				...
				if (!Character.isLetter((char) ch))
				{
					...
					u = u.replaceAll("[\\p{Punct}&&[^'-]]+", "");
					u = u.replaceAll("-", "");
					// replace most common English contractions
					u = u.replaceAll("(?:'(?:[tdsm]|[vr]e|ll))+\\b", "");
					...
					if (u.equals("") || stopWords.contains(u) || u.matches("[-+]?\\d*\\.?\\d+"))
						continue;
					if (!wordCounts.containsKey(u)) {
						wordCounts.put(u, 1);
					} else {
						wordCounts.put(u, wordCounts.get(u) + 1);
					}
