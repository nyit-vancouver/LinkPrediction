...
public class DiggMetaPathUUU {
	...
	private static Map<Integer, ArrayList<Integer>> user_friends_map = new HashMap<Integer, ArrayList<Integer>>();    
	...
	public static int pathCount_UUU(int u1, int u2){
		...
		if (!user_friends_map.containsKey(u1)){
			//System.out.println("user " + u1 + "is not in the map!");
			return 0;
		}
		if (!user_friends_map.containsKey(u2)){
			//System.out.println("user " + u2 + "is not in the map!");
			return 0;
		}

		ArrayList<Integer> A = user_friends_map.get(u1);
		ArrayList<Integer> B = user_friends_map.get(u2);
		for (int u3: A)
			if (B.contains(u3))
				pathCount++; // number of shared items
	...
	{	 
		...
		String usre_friends_file_name = "Digg/" + intervals + "intervals_friends/digg_friends_" + currentInterval + "of" + intervals + ".txt"; // user-story and story-user infor for current time
		...
		try{
			...
			BufferedReader userfriendsFile = new BufferedReader(new FileReader(usre_friends_file_name));
			// file format example (3 nodes)
			//3
			//0,0
			//1,3:2,1:3,1:4,1
			//...			
			int numOfNodes = Integer.parseInt(userfriendsFile.readLine()); // reading numOfNodes
			for (int i=0; i < 279631; i++){  // considering only users and not stories
				ArrayList<Integer> storiesList = new ArrayList<Integer>();
				currentLine = userfriendsFile.readLine();
				from = 0;
				...
				for (int j=0; j < Integer.parseInt(numOfConnectedNodes) ; j++){
					...
					if (user_friends_map.containsKey(userIndex))
						a = user_friends_map.get(userIndex);
					a.add(friendIndex);	
					user_friends_map.put(userIndex, a);
				}
			}
