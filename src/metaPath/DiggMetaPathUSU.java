...
public class DiggMetaPathUSU {
	...
	private static short[][] story_users = new short[3553][336225];

	public static int pathCount_USU(int u1, int u2){
		int pathCount = 0;

		for(int l=0;l<3553; l++)
			pathCount += user_stories[u1][l] * story_users[l][u2];
	...
	{	 
		...
		try{
			...
			while ((currentLine = labels.readLine()) != null){
				...
				bwUserstoryLabel.write(pathCount_USU(sourceNode, destNode)+"\n");
